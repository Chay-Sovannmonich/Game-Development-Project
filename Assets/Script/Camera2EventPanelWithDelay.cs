...
public class Camera2EventPanelWithDelay : MonoBehaviour
{
    [Header("=== TIMING SETTINGS ===")]
    public float delayBeforeShow = 2f;  // 2-second delay before showing
    public float displayTime = 1f;      // How long to show the panel
    ...
    void Start()
    {
        Debug.Log("Camera2EventPanel initializing with 2-second delay...");
        ...
        Debug.Log($"Panel will show after {delayBeforeShow}s delay for {displayTime}s ONCE when Camera2 activates.");
    }
    ...
    void OnCameraActivated()
    {
        ...
        Debug.Log($"Camera2 activated! Waiting {delayBeforeShow}s before showing panel...");
        ...
        // Start delay then show panel
        currentCoroutine = StartCoroutine(DelayThenShowPanel());
    }
    
    IEnumerator DelayThenShowPanel()
    {
        // WAIT 2 SECONDS
        Debug.Log($"Waiting {delayBeforeShow} seconds...");
        yield return new WaitForSeconds(delayBeforeShow);
        
        // Show panel with timer
        yield return StartCoroutine(ShowAndHidePanelOnce());
    }
    
    IEnumerator ShowAndHidePanelOnce()
    {
        Debug.Log($"Delay complete! Showing panel for {displayTime} seconds...");
    ...
    // Public method to manually trigger panel with delay
    public void ShowWithDelay(float customDelay = -1, float customDuration = -1)
    {
        if (hasShownOnce)
        {
        ...
        hasShownOnce = true;
        float delay = (customDelay > 0) ? customDelay : delayBeforeShow;
        float duration = (customDuration > 0) ? customDuration : displayTime;
        
        currentCoroutine = StartCoroutine(DelayThenShowCustom(delay, duration));
    }
    
    IEnumerator DelayThenShowCustom(float delay, float duration)
    {
